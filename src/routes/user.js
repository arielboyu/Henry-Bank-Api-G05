...
server.put('/alta/:id', upload.single('file'), async (req, res) => {
  const { id } = req.params;
  const { firstName, lastName, mobile, street, streetNumber, city, province, country, birthdate, typeDNI, photoURL, DNI } = req.body;

  if (!firstName || !lastName || !mobile ||  !street || !streetNumber || !city || !province || !country || !birthdate || !typeDNI || !DNI )
  return  res.status(400).json({ Error: "Must fill all the fields" }); //Valida que esten todos los campos completos
